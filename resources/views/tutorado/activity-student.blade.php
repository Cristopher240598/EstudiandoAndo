            ...
            <div class="row" style="min-width: 100%;max-width: 100%;">
                ...
                <div class="col" style="min-width: 100%;max-width: 100%;">
                    <div class="p-5" style="padding: 30px!important;">
                        ...
                        <form class="user" method="POST" action="{{ route('tutorado.respuesta-actividad', ['idActividad' => $actividadTutorado->id]) }}" enctype="multipart/form-data">
                            @csrf
                            @foreach($actividadTutorado->preguntasTutorado as $pregunta)
                                <div class="form-group">
                                    ...
                                    <textarea class="form-control form-control-user @error('respuesta' . $pregunta->id) is-invalid @enderror" id="respuesta{{$pregunta->id}}" name="respuesta{{ $pregunta->id }}" name="respuesta{{ $pregunta->id }}" placeholder="Respuesta" style="font-size: 18px;color: rgb(0,0,0);" required autocomplete="off" autofocus>{{ old('respuesta'.$pregunta->id) }}</textarea>
                            ...
                            @endforeach
                            <button class="btn btn-primary btn-block text-white btn-user" type="submit" role="button" style="font-size: 18px;max-width: 55%;margin: 0 auto;">Terminar</button>
                        </form>
                    </div>
                </div>
            </div>
