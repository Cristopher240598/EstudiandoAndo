...
@section('content')
    <div class="card shadow mb-4">
        <div class="card-header text-center">
            <h6 class="text-primary font-weight-bold m-0" style="font-size: 25px;">Actividades de {{ $tutorado->usuario->nombre . ' ' . $tutorado->usuario->apellidoPaterno . ' ' . $tutorado->usuario->apellidoMaterno}}</h6>
        </div>
        <div class="card-body">
                    ...
                    <div class="col col-activities" style="margin-bottom: 10px;">
                        <div class="card">
                            @if($actividad->estatus == null)
                                <a href="{{ route('tutor.actividad-tutorado',['id' => $actividad->actividadTutorado->id]) }}">
                            @else
                            <a href="{{ route('tutor.respuesta-actividad-tutorado',['id' => $actividad->actividadTutorado->id, 'idTutorado' => $actividad->id_tutorado]) }}">
                            @endif
                            ...
                            <div class="card-body text-center">
                                <h6>
                                    @if($actividad->estatus == null)
                                        <a class="event_title" href="{{ route('tutor.actividad-tutorado',['id' => $actividad->actividadTutorado->id]) }}" style="font-size: 22px;">{{ $actividad->actividadTutorado->nombre }}</a>
                                    @else
                                        <a class="event_title" href="{{ route('tutor.respuesta-actividad-tutorado',['id' => $actividad->actividadTutorado->id, 'idTutorado' => $actividad->id_tutorado]) }}" style="font-size: 22px;">{{ $actividad->actividadTutorado->nombre }}</a>
                                    @endif
                                </h6>
